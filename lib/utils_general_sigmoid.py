...
def train_model(x, y, x_control, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh, width, gamma=None):
    ...
    if apply_fairness_constraints == 0:
        ...
        constraints = get_constraint_list_cov(x, y, x_control, sensitive_attrs, sensitive_attrs_to_cov_thresh, width)      
    ...
    if apply_accuracy_constraint == 0: #its not the reverse problem, just train w with cross cov constraints
        ...
        def cross_cov_abs_optm_func(weight_vec, x_in, x_control_in_arr):
            ...
            phi_d_theta = phi(d_theta, width)            # mapping the distance 
...
def test_sensitive_attr_constraint_cov(model, x_arr, y_arr_dist_boundary, x_control, thresh, verbose, width):
...
def print_covariance_sensitive_attrs(model, x_arr, y_arr_dist_boundary, x_control, sensitive_attrs, width):
    ...
    for attr in sensitive_attrs:
        ...
        if bin_attr:
            cov = thresh - test_sensitive_attr_constraint_cov(None, x_arr, arr, np.array(attr_arr), thresh, False, width)
            ...
            for attr_val, ind in index_dict.items():
                ...
                cov = thresh - test_sensitive_attr_constraint_cov(None, x_arr, arr, t, thresh, False, width)
...
def get_constraint_list_cov(x_train, y_train, x_control_train, sensitive_attrs, sensitive_attrs_to_cov_thresh, width):
    ...
    for attr in sensitive_attrs:
        ...
        if index_dict is None: # binary attribute
            ...
            c = ({'type': 'ineq', 'fun': test_sensitive_attr_constraint_cov, 'args':(x_train, y_train, attr_arr_transformed,thresh, False, width)})
            constraints.append(c)
            ...
            for attr_val, ind in index_dict.items():
                ...
                t = attr_arr_transformed[:,ind]
                c = ({'type': 'ineq', 'fun': test_sensitive_attr_constraint_cov, 'args':(x_train, y_train, t ,thresh, False, width)})
                constraints.append(c)
