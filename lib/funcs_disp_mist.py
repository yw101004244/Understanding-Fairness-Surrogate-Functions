...
def train_model_disp_mist(x, y, x_control, loss_function, EPS, cons_params=None):
    ...
    if loss_function == "logreg":
        ...
        loss = sum(  cvxpy.logistic( cvxpy.multiply(-y, x*w) )  ) / num_points # we are converting y to a diagonal matrix for consistent
    ...
    try:
        ...
        tau, mu = 5000, 2 # default dccp parameters, need to be varied per dataset
    ...
    for attr in sensitive_attrs_to_cov_thresh.keys():
        ...
        if index_dict is None: # binary attribute, in this case, the attr_arr_transformed is the same as the attr_arr
            ...
            for v in set(attr_arr):
                ...
                #################################################################
                # #DCCP constraints
                dist_bound_prod = y_train[idx] * np.dot(x_train[idx], w.value) # y.f(x)
                
                cons_sum_dict[0][v] = sum( a_min(0, dist_bound_prod) ) * (s_val_to_avg[0][v].value / len(x_train))  # avg misclassification distance from boundary
                cons_sum_dict[1][v] = sum( a_min(0, np.multiply( (1 - y_train[idx])/2.0, dist_bound_prod) )) * (s_val_to_avg[1][v].value / sum(y_train == -1)) # avg false positive distance from boundary (only operates on the ground truth neg dataset)
                cons_sum_dict[2][v] = sum( a_min(0, np.multiply( (1 + y_train[idx])/2.0, dist_bound_prod) )) * (s_val_to_avg[2][v].value / sum(y_train == +1)) # avg false negative distance from boundary
                #################################################################
...
def plot_fairness_acc_tradeoff(x_all, y_all, x_control_all, loss_function, cons_type):
    ...
    ax.set_xlim([cvxpy.min(cov_range), cvxpy.max(cov_range)])
    ...
    ax.set_xlim([cvxpy.min(p_rule_arr), cvxpy.max(cvxpy.max(p_rule_arr), 100)])
    plt.xlabel('P% rule')
    plt.ylabel('Accuracy')
