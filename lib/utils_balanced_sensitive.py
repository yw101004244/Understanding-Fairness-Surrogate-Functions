...
def train_model(x, y, x_control, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh, term, a_c_index, b_d_index, gamma=None):
    ...
    if apply_fairness_constraints == 0:
        ...
        constraints = get_constraint_list_cov(x, y, x_control, sensitive_attrs, sensitive_attrs_to_cov_thresh, term, a_c_index, b_d_index)      
    ...
    if apply_accuracy_constraint == 0: #its not the reverse problem, just train w with cross cov constraints
        ...
        def cross_cov_abs_optm_func(weight_vec, x_in, x_control_in_arr):
            ...
            # assert len(d_theta) == len(a_c_index), "\phi error 1"
            phi_d_theta = phi(d_theta, term, a_c_index, b_d_index)            # mapping the distance 
...
def compute_cross_validation_error(x_all, y_all, x_control_all, num_folds, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh_arr, gamma=None):
    ...
    def train_test_single_fold(train_data, test_data, fold_num, output_folds, sensitive_attrs_to_cov_thresh):
        ...
        cov_dict_test = print_covariance_sensitive_attrs(None, x_test, distances_boundary_test, x_control_test, sensitive_attrs, term)
        ...
        cov_dict_train = print_covariance_sensitive_attrs(None, x_train, distances_boundary_train, x_control_train, sensitive_attrs, term)
...
def general_sigmoid(x, w):
    return 2*sigmoid(w*x)-1


def phi(x, t, a_c_index, b_d_index, width=2, height=1):

    '''based on sensitive features'''
    # assert len(x) == len(a_c_index), "\phi error"


    '''balance linear'''
    #  x[a_c_index] do not change when applying balanced linear surrogate
    # x[a_c_index] = x[a_c_index] # do not use this line, but use it as a mark
    x[b_d_index] = t * x[b_d_index]


    '''balance general sigmoid'''
    # x[a_c_index] = general_sigmoid(x[a_c_index], width)
    # x[b_d_index] = t * general_sigmoid(x[b_d_index], width)
...
def test_sensitive_attr_constraint_cov(model, x_arr, y_arr_dist_boundary, x_control, thresh, verbose, term, a_c_index, b_d_index):
    ...
    # assert len(arr) == len(a_c_index), "\phi error 2"
    arr = phi(arr, term, a_c_index, b_d_index)
...
def print_covariance_sensitive_attrs(model, x_arr, y_arr_dist_boundary, x_control, sensitive_attrs, term, a_c_index, b_d_index):
    ...
    for attr in sensitive_attrs:
        ...
        if bin_attr:
            cov = thresh - test_sensitive_attr_constraint_cov(None, x_arr, arr, np.array(attr_arr), thresh, False, term, a_c_index, b_d_index)
            ...
            for attr_val, ind in index_dict.items():
                temp = attr_arr_transformed[:,ind]
                cov = thresh - test_sensitive_attr_constraint_cov(None, x_arr, arr, temp, thresh, False, term, a_c_index, b_d_index)
...
def get_constraint_list_cov(x_train, y_train, x_control_train, sensitive_attrs, sensitive_attrs_to_cov_thresh, term, a_c_index, b_d_index):
    ...
    for attr in sensitive_attrs:
        ...
        if index_dict is None: # binary attribute
            ...
            c = ({'type': 'ineq', 'fun': test_sensitive_attr_constraint_cov, 'args':(x_train, y_train, attr_arr_transformed,thresh, False, term, a_c_index, b_d_index)})
            constraints.append(c)
            ...
            for attr_val, ind in index_dict.items():
                ...
                c = ({'type': 'ineq', 'fun': test_sensitive_attr_constraint_cov, 'args':(x_train, y_train, t ,thresh, False, term, a_c_index, b_d_index)})
                constraints.append(c)
...
def compute_term(w, x_test, sensitive_index):
    ...
    S_A = np.sum(D_theta[A_INDEX==1])
    S_B = np.sum(D_theta[B_INDEX==1])
    S_C = np.sum(D_theta[C_INDEX==1])
    S_D = np.sum(D_theta[D_INDEX==1])
    ...
    term = ( (B+D)*(S_A-S_C)+(B*C-A*D) ) / ( (A+C)*(S_B-S_D) ) # balance z
    # term = ((A+C)*S_B-(B+D)*S_A) / ((A+C)*S_D-(B+D)*S_C) # balance y
...
def compute_term_removed(d_theta, a_c_index, b_d_index, n):
    ...
    # numerator = (B+D)*(S_A-S_C)+2*(B*C-A*D)
    # denominator = (A+C)*(S_B-S_D)
...
def draw_boxplot(w, x_test):
    ...
    ''' after mapping '''
    # d_theta = phi(d_theta, t) # notice the param list has changed ("t")
    # d_theta_positive = d_theta[np.where(d_theta >= 0)]
    # d_theta_negative = d_theta[np.where(d_theta < 0)]
    # rt = plt.boxplot([d_theta_positive,d_theta_negative],
    #                   labels = labels,
