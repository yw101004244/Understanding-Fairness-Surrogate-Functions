...
def _hinge_loss(w, X, y):
    ...
    wtw = np.linalg.norm(w, ord=2, axis=None, keepdims=False)
    wTw = wtw**2
    
    return sum(yz) + 0.1*wTw
...
def _logistic_loss(w, X, y, return_arr=None):
    ...
    if return_arr == True:
        ...
        # out = -np.sum(log_logistic(yz)) + 0.01*wTw
...
def _negative_sigmoid(w, X, y, t=1):
    f = y * np.dot(X,w)

    # wtw = np.linalg.norm(w, ord=2, axis=None, keepdims=False)
    # wTw = wtw**2
    # return np.sum(sigmoid(-t*f)) + 0.1*wTw

    return np.sum(sigmoid(-t*f))