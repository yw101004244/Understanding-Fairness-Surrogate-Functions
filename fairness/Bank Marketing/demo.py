...
def test_bank_data():
	...
	""" Load the bank data """
	X, y, x_control = load_bank_additional_data() # set the argument to none, or no arguments if you want to test with the whole data -- we are subsampling for performance speedup
	ut.compute_violation_of_disparate_impact(x_control['age'], y) # compute the p-rule in the original data
	...
	# 	apply_accuracy_constraint = 0
	# 	sep_constraint = 0
	# 	sensitive_attrs_to_cov_thresh = {'age':thres}
	# 	print(thres)
	...
	for i in range(0,20):
		...
		sep_constraint = 0
		gamma = i/100 # gamma controls how much loss in accuracy we are willing to incur to achieve fairness -- increase gamme to allow more loss in accuracy
		print(gamma)
