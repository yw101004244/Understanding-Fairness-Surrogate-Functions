...
#     print("\n\n")
#     print("Now width={:.2f}".format(width))
#     print("\n\n")
...
for train_index, test_index in kf.split(X):
    ...
    '''INDEX'''
    sensitive_array_test = x_control_test['sex']
    A_C_INDEX_TEST = np.array(sensitive_array_test==0).astype(int)
    B_D_INDEX_TEST = np.array(sensitive_array_test==1).astype(int)
    N_TEST = sensitive_array_test.shape[0]

    sensitive_array = x_control_test['sex']
    A_C_INDEX = np.array(sensitive_array==0).astype(int)
    B_D_INDEX = np.array(sensitive_array==1).astype(int)
    N = sensitive_array_test.shape[0]
    ...
    for i in range(0,1):
        ...
        if p_f_cons < 0.1 and acc_f_cons > 0.75: 
            ...
            method_list_balance.append('B-Linear')
