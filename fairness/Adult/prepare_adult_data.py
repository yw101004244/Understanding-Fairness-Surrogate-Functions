...
def load_adult_data(load_data_size=None):
    ...
    # adult data comes in two different files, one for training and one for testing, 
    # however, we will combine data from both the files
    data_files = [r'C:\Users\yao\Desktop\understanding\fairness\Adult\adult.data',
                  r'C:\Users\yao\Desktop\understanding\fairness\Adult\adult.test']


    '''initiate some data structures'''
    X = []
    y = []
    x_control = {}
    attrs_to_vals = {} # will store the values for each attribute for all users.
                       # It is a dictionary, in which keys are the names of the attibutes
                       # and values are the corresponding value. 
                       # The structure of x_control is similar.
    ...
    for f in data_files:
        ...
        for line in open(f):
            ...
            '''get the class label'''
            class_label = line[-1] # the label is in the last position
